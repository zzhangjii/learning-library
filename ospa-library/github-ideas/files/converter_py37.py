...
def assure_path_exists():
    ...
    if platform.system() == "Linux" or platform.system() == "Linux2" or platform.system() == "Darwin":
        if not os.path.exists(entry1.get()):
            MsgBox = tkinter.messagebox.askquestion("Error", "Directory structure does not exist. Do you want to create the directory structure?")
            if MsgBox == 'yes':
                os.makedirs(git_repo)
                tkinter.messagebox.showinfo("Information", "Directory structure created. You must clone your dev repository")
                flag = 1
        ...
        if not os.path.exists(entry1.get()):
            ...
            if MsgBox == 'yes':
                os.makedirs(git_repo)
                tkinter.messagebox.showinfo("Information", "Directory structure created. You must initiate your dev repository")
                flag = 1
    ...
    if status_init != 1:
        ...
        if platform.system() == "Linux" or platform.system() == "Linux2" or platform.system() == "Darwin":
            try:
                ...
                tkinter.messagebox.showinfo("Error", "Invalid path or repository")
        ...
        if checkBoxA.get() == True:
            tkinter.messagebox.showinfo("Information", "Convertion complete. Uploading content")
        else:
            tkinter.messagebox.showinfo("Information", "Convertion complete.")
        ...
        if checkBoxA.get() == True:
            ...
            tkinter.messagebox.showinfo("Information", "Content updated")
...
def ospa_help():
    tkinter.messagebox.showinfo("Information - Author: Alejandro Casas <alejandro.casas@oracle.com>", "Oracle Sales and Partner Academy \
...
def cloneRepo(url, path, user):
    try:
        ...
        tkinter.messagebox.showinfo("Information", "Github repository has been cloned")
    except OSError:
        tkinter.messagebox.showinfo("Error", "Invalid path or repository")
...
def push(path):
    try:
        ...
        subprocess.call(cmd, shell=True)
        tkinter.messagebox.showinfo("Information", "Github repository has been updated")
...
def pull(path):
    try:
        ...
        cmd = 'git pull'
        subprocess.call(cmd, shell=True)
        tkinter.messagebox.showinfo("Information", "Local repository has been updated")
