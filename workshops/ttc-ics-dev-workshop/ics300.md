...
- **What do you want to call your endpoint?:** Enter the name `lookupItemName` as the service invocation name.
...
**2.2.5** Copy the SQL statement provided inline here with a parameter for `ITEM_NUMBER` specified:

`select LOOKUP_VALUE from TTC_LOOKUP where LOOKUP_KEY = #ITEM_NUMBER`
...
**2.2.8** Observe that the orchestration's design pallette is populated with the invoke _lookupItemName_ you just created along with the mapping needed to invoke this adapter.
...
**2.2.9** Click on the *mapping symbol* for the _lookupItemName_ mapping - this will bring up 3 icons for activities we can do to the mapping.  
...
**2.2.10** Edit the mapping by clicking on the little circle to the right of the `ItemID` source variable and then drag it on top of the little circle just to the left of the `ITEM_NUMBER` target variable.
...
- In the Source variable section, click on the chevron to the left of the `lookupItemNameOutput` variable to expand it.
...
**2.3.9** Type in the string `Lookup Value = ` (no surrounding quotes needed) in the editable textbox for the first argument for the concat function.
...
**2.4.4** Click on the *ItemID* variable in the Source variable section.  The editor will show the metadata for this variable such as type, path, etc.

**2.4.5** Next, click on the `Shuttle Icon` to move the *ItemID* variable over to be a new `Tracking Field`.

**2.4.6** Observe that the *ItemID* variable has now been added as a `Tracking Field`.  The source variable name is also added as the `Tracking Name` by default – since *ItemID* means something and is suitable for people to read in the monitoring tab we’ll keep it. If the comment variable was something random like *C2EF*, we would want to change it and give it a human-readable tracking name.
...
**3.21** Change the value of the **ItemID** from _2155_ to _1777_ and see if you get a different lookup result?  What do you find?

NOTE 1: Other lookup IDs to try are _155_, _174762_, _4816_, 137_, and _12019_

NOTE 2: We could have put logic in the orchestration to give a response like `No Value found for lookup ID XXX` - that's left as an exercise to the participant.
