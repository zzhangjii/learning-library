...
- In Brackets, choose the **public/js/alphaOffice.js** file to edit.  You will be pasting the URLs of the two microservices into the code in order to access these services.  Paste the URL for the AlphaOfficeMySQLREST microservice as the value for the Javascript variable dbServiceURL.  Paste the URL for the JavaTwitterMicroservice microservice as the value for the Javascript variable tweetServiceBaseURL.
...
- Go to the Cloud Developer Service console.  Click on **Code** and on the **Refs** tab. Note the recently pushed update with the commit message text you just entered. Click **Master** to commit the change to the master branch.
