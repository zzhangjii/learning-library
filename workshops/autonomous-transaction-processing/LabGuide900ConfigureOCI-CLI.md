...
- A keypair used for signing API requests, with the public key uploaded to Oracle. Only the user calling the API should possess the private key. See [Cofigure the CLI](https://docs.cloud.oracle.com/iaas/Content/API/SDKDocs/cliconfigure.htm)
...
#### Note the Docker image comes with pre-installed OCI CLI. Once you go through the instructions to install OCI CLI you can start with configuring OCI setup config file in STEP 2: Configure OCI CLI in this lab
