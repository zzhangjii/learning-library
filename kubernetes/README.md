...
- Hm! No tweets are displayed in the right pane of the details. Looks like we haven't deployed that microservice to our cluster yet. Let's do that now.

  ![](images/3.png)

### Deploy Twitter Feed Microservice

- We can create a new Kubernetes deployment in a few ways. One common one is using the command line utility **kubectl**, which allows interactions with Kubernetes to be automated. Today, however, we will use the Kubernetes dashboard to do a one-off deployment of the **twitter-feed**. Open the dashboard tab in your browser and click **Create** in the upper right corner.

  ![](images/5.png)

- The create feature allows you to specify the details of a deployment (or other resources) by pasting in text, uploading a file, or filling out a form. Today we'll paste in some text that defines our twitter feed microservice. Go to **[https://raw.githubusercontent.com/derekoneil/demo-labs/master/kubernetes/twitter-feed.yml](https://raw.githubusercontent.com/derekoneil/demo-labs/master/kubernetes/twitter-feed.yml)** to find the YAML that we'll use. **Select all of the text, copy it, and paste it into the text box** on the Kubernetes dashboard create dialog. Then click **Upload**.
...
- Here is the whole YAML file that we uploaded to Kubernetes:
...
- Let's look at some of the sections in more detail:
  ...
  metadata:
    ...
    labels:
      commit: 321c9efce6a0fce712e80b6d999cc01c63781c20
  ```
  - This tells us that we are looking at a deployment object, and we have provided metadata in the form of a name and labels
  ...
  spec:
    ...
    selector:
      matchLabels:
        app: twitter-feed
        ```
    - The start of the deployment specification indicates that we want two twitter-feed pods (replicas) running at all times, and that this deployment should manage any pods with the label: "app: twitter-feed".
  ...
  template:
    metadata:
      labels:
        ...
        color: green
        ```
    - Next we see the beginning of a (pod) template definition inside the deployment definition. We've given it some labels as well. One of those labels is "app: twitter-feed", so we know that any pods created by this template will be managed by this deployment (which is what we want). Note the "color: green" label, which can be used to facilitate Blue/Green deployments, which is a topic covered in the Container Native Development workshop (more details on that later).
  ...
  spec:
    ...
    - name: twitter-feed
      ...
      - name: twitter-feed
        ...
        protocol: TCP
        ```
    - Finally we see the specification for the pods that this deployment will ultimately create. Here is where we define the Docker container image that the pods will run, as well as the ports that the application will need open.

    - The rest of the deployment spec deals with Volumes, which allow us to mount physical or virtual disks inside our pods. That topic is also covered in the Container Native workshop.
  ...
  metadata:
    ...
    labels:
      ...
      commit: 321c9efce6a0fce712e80b6d999cc01c63781c20
    ```
    - Following the deployment details we see two dashes, and then another specification, this time for a Service. This service (of course) has a name and some other metadata defined.
  ...
  spec:
    ...
    type: ClusterIP
  ```
    - It also has a specification section that defines it. We can see that some port mapping is done to redirect traffic on port 30000 of the service IP to port 8080 of the pod IP (where our application is listening). The selector section tells the service which pods make up this service, in this case only ones with both the labels "app: twitter-feed" and "color: green". It also specifies that this service is of type ClusterIP, meaning that it is only accessible from inside the Kubernetes cluster. This is ok, since only the product catalog application needs to invoke it.
...
- Note that on the details page we can see all of the labels that we looked at in the pod template spec, as well as all of the events that this pod has triggered. Let's make sure our twitter feed is functioning properly. Click the **Exec** button in the top menu bar.

  ![](images/11.png)

- The Exec function opens a shell into the pod that we were just inspecting, which allow us to test and debug directly inside the Docker container. Run the following command to invoke the local Java web server that hosts the twitter feed:
...
- You should see some JSON data being returned by our twitter feed service. Our microservice has been deployed successfully! Let's see if it shows up in the product catalog.
...
- Looks like the product catalog is able to pull data from the twitter feed microservice successfully. But how did it know how to find the twitter-feed pods, which we just created? The answer is the twitter-feed service that we created along with our deployment. Let's take a look.
...
- One of the major benefits to containerizing your application is the ease of scaling it out or in to accommodate demand. Let's scale out our product catalog application to see how easily it is.

- Switch to the Kubernetes dashboard tab in your browser. Click on the **product-catalog-app** deployment.

  ![](images/13.png)

- In the **New Replica Set** table, look in the Pods column to see how many instances of the application are currently running. 2/2 means we have two currently running and that we have requested 2 total.
...
- We worked with pre-built docker containers and a pre-installed Kubernetes cluster today. To learn how to start from bare VMs and source code, join us for a virtual, in-person, or self-service workshop where we explore container native development in depth. Visit [https://go.oracle.com/container-native-development](https://go.oracle.com/container-native-development) to learn more.
